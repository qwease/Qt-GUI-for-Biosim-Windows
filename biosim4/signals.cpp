...
#pragma omp critical
    {
        visitNeighborhood(loc, radius, [layerNum](Coord loc) {
            if (pheromoneSignals[layerNum][loc.x][loc.y] < SIGNAL_MAX) {
                pheromoneSignals[layerNum][loc.x][loc.y] =
                        std::min<unsigned>(SIGNAL_MAX,
                                                                              pheromoneSignals[layerNum][loc.x][loc.y] + neighborIncreaseAmount);
            }
        });
        ...
        if (pheromoneSignals[layerNum][loc.x][loc.y] < SIGNAL_MAX) {
            pheromoneSignals[layerNum][loc.x][loc.y] =
                        std::min<unsigned>(SIGNAL_MAX,
                                                                          pheromoneSignals[layerNum][loc.x][loc.y] + centerIncreaseAmount);
        }
    }
}
...
void Signals::fade(unsigned layerNum)
{
    ...
    for (int16_t x = 0; x < p.sizeX; ++x) {
        for (int16_t y = 0; y < p.sizeY; ++y) {
            if (pheromoneSignals[layerNum][x][y] >= fadeAmount) {
                pheromoneSignals[layerNum][x][y] -= fadeAmount;  // fade center cell
            } else {
                pheromoneSignals[layerNum][x][y] = 0;
            }
        }
    }
}

...
void Indiv::serialize(std::ostream& os) const {
    // Write the size of the genome
    size_t size = genome.size();
    os.write(reinterpret_cast<const char*>(&size), sizeof(size));

    // Write the genome data
    os.write(reinterpret_cast<const char*>(genome.data()), size * sizeof(Gene));
}

void Indiv::deserialize(std::istream& is) {
    // Read the size of the genome
    size_t size;
    is.read(reinterpret_cast<char*>(&size), sizeof(size));

    // Resize the genome and read the data
    genome.resize(size);
    is.read(reinterpret_cast<char*>(genome.data()), size * sizeof(Gene));
}
